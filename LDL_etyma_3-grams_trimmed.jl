...
S_DUT = npzread(joinpath(@__DIR__, "data_forLDL", "S_DUT_etymamatrix_trimmed.npy"))
S_ENG = npzread(joinpath(@__DIR__, "data_forLDL", "S_ENG_etymamatrix_trimmed.npy"))
S_GER = npzread(joinpath(@__DIR__, "data_forLDL", "S_GER_etymamatrix_trimmed.npy"))

# read in Datasets that were processed in Python
etyma_DUT = DataFrame!(CSV.File(joinpath(@__DIR__, "data_forLDL", "etyma_dutchdata_trimmed.tsv"), delim="	"));
etyma_GER = DataFrame!(CSV.File(joinpath(@__DIR__, "data_forLDL", "etyma_germandata_trimmed.tsv"), delim="	"));
etyma_ENG = DataFrame!(CSV.File(joinpath(@__DIR__, "data_forLDL", "etyma_englishdata_trimmed.tsv"), delim="	"));
...
cue_obj_GER = JudiLing.make_cue_matrix(etyma_GER, grams=3, target_col="Sound_Classes", tokenized=true, sep_token=".",
    keep_sep=true,);
cue_obj_DUT = JudiLing.make_cue_matrix(etyma_DUT, grams=3, target_col="Sound_Classes", tokenized=true, sep_token=".",
    keep_sep=true,);
cue_obj_ENG = JudiLing.make_cue_matrix(etyma_ENG, grams=3, target_col="Sound_Classes", tokenized=true, sep_token=".",
    keep_sep=true,);
