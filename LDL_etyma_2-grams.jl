...
Pkg.add(url="https://github.com/MegamindHenry/JudiLing.jl",
	rev="pycall_optional") 
...
S_DUT = npzread(joinpath(@__DIR__, "data", "S_DUT_etymamatrix.npy"))
S_ENG = npzread(joinpath(@__DIR__, "data", "S_ENG_etymamatrix.npy"))
S_GER = npzread(joinpath(@__DIR__, "data", "S_GER_etymamatrix.npy"))

# read in Datasets that were processed in Python
etyma_DUT = DataFrame!(CSV.File(joinpath(@__DIR__, "data", "etyma_dutchdata.tsv"), delim="	"));
etyma_GER = DataFrame!(CSV.File(joinpath(@__DIR__, "data", "etyma_germandata.tsv"), delim="	"));
etyma_ENG = DataFrame!(CSV.File(joinpath(@__DIR__, "data", "etyma_englishdata.tsv"), delim="	"));
...
cue_obj_GER = JudiLing.make_cue_matrix(etyma_GER, grams=2, target_col="Sound_Classes", tokenized=true, sep_token=".",
    keep_sep=true,);
cue_obj_DUT = JudiLing.make_cue_matrix(etyma_DUT, grams=2, target_col="Sound_Classes", tokenized=true, sep_token=".",
    keep_sep=true,);
cue_obj_ENG = JudiLing.make_cue_matrix(etyma_ENG, grams=2, target_col="Sound_Classes", tokenized=true, sep_token=".",
    keep_sep=true,);
